...
export const Title = styled.h1<{ $titleStyle?: TitleStyleEntity }>`
  ...
  margin-bottom: ${({ $titleStyle }) =>
    $titleStyle?.marginBottom ? $titleStyle.marginBottom : 0}px;
...
export const Content = styled.div<{ $contentStyle?: ContentStyleEntity }>`
  ...
  margin-left: ${({ $contentStyle }) =>
    $contentStyle?.marginLeft ? $contentStyle.marginLeft : 0}px;
`;
